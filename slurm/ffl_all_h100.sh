...
#SBATCH --job-name=ch_ffl_all_bs4x16  # Job name
#SBATCH --output=./slurm/runs/ffl_all_bs4x16_ch100.log       # Standard output and error log
#SBATCH --error=./slurm/runs/ffl_all_bs4x16_ch100.log         # Error log
...
torchrun --nproc_per_node=4 scripts/train.py log_to_wandb=true host=jz run_type=release host.multi_gpu=true checkpoint=latest experiment=ffl_fusion experiment.name=v0_all_bs4x16 country=all
