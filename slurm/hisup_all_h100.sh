...
#SBATCH --job-name=c_hisup_all_bs4x16  # Job name
#SBATCH --output=./slurm/runs/hisup_all_bs4x16_c.log       # Standard output and error log
#SBATCH --error=./slurm/runs/hisup_all_bs4x16_c.log         # Error log
...
torchrun --nproc_per_node=4 scripts/train.py log_to_wandb=true host=jz run_type=release host.multi_gpu=true checkpoint=latest experiment=hisup_fusion experiment.name=v0_all_bs4x16 country=all
