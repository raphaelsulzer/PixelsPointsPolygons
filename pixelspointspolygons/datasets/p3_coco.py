...
class P3Dataset(Dataset):
    ...
    def sort_vertices_by_coordinates(self, coords_seq: list):
        arr = np.array(coords_seq).reshape(-1,2)
        # sort by x, then by y
        sorted_idxs = np.lexsort((arr[:,1], arr[:,0]))
        sorted_arr = arr[sorted_idxs]
        
        return sorted_arr, sorted_idxs
    ...
    def __getitem__pix2poly(self, index: int):
        ...
        coords_seqs, rand_idxs = self.tokenizer(corner_coords, shuffle=False)
        
        corner_coords = np.reshape(coords_seqs[1:-1], (-1,2))

        if self.cfg.experiment.model.vertex_order == "coord":
            corner_coords, sorted_idxs = self.sort_vertices_by_coordinates(corner_coords)
        elif self.cfg.experiment.model.vertex_order == "random":
            sorted_idxs = np.arange(len(corner_coords))
            np.random.shuffle(sorted_idxs)
            corner_coords = corner_coords[sorted_idxs]
        elif self.cfg.experiment.model.vertex_order == "gt":
            sorted_idxs = np.arange(len(corner_coords))
        else:
            raise ValueError(f"Unknown vertex ordering {self.cfg.experiment.model.vertex_order}")
        
        coords_seqs = [coords_seqs[0]]+list(corner_coords.flatten())+[coords_seqs[-1]]

        coords_seqs = torch.LongTensor(coords_seqs)
        perm_matrix = self.shuffle_perm_matrix_by_indices(perm_matrix, sorted_idxs)
